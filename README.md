...
- **ภาพรวม** KPI รวมทุกไซต์ (PV, โหลด, กริดซื้อ/ขาย, แบตชาร์จ/คาย, พลังงานวันนี้) กราฟกำลังไฟรวมย้อนหลัง 3-48 ชม. และการ์ดต่อไซต์
- **ไซต์/ลูกค้า** ตารางลูกค้า โทร ที่อยู่ สถานะ ปุ่ม **แก้ไข / เพิ่มไซต์** แก้ชื่อไซต์ ชื่อลูกค้า โทร ที่อยู่ หมายเหตุ และเลือกอุปกรณ์ในไซต์ได้จากหน้าเว็บ บันทึกลง `sites.yaml` ข้าง config.yaml (ไม่ทับ config.yaml และมีผลทันที) หน้าไซต์แต่ละรายมี dashboard ของตัวเอง คือ KPI, กราฟกำลังไฟและ SOC ย้อนหลัง, ตารางอุปกรณ์ และ alarm ค้าง
...
```bash
python -m solar_relay.probe --scan                              # ยังไม่รู้ IP: กวาดทั้งวง /24 ของเครื่องนี้ แล้วตรวจทุกตัวที่เจอ
